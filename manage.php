...
class Manage
{
	...
	public function storeCustomerOrderInvoice($orderdate,$cust_name,$ar_tqty,$ar_qty,$ar_price,$ar_pro_name,$sub_total,$discount,$net_total,$paid,$due){
		$pre_stmt = $this->con->prepare("INSERT INTO 
			`sales`(`customer_name`, `order_date`, `sub_total`,
			  `discount`, `net_total`, `paid`, `due`) VALUES (?,?,?,?,?,?,?)");
		$pre_stmt->bind_param("ssdddddds",$cust_name,$orderdate,$sub_total,$discount,$net_total,$paid,$due);
		$pre_stmt->execute() or die($this->con->error);
		...
		if ($invoice_no != null) {
			for ($i=0; $i < count($ar_price) ; $i++) {
				...
				if ($rem_qty < 0) {
					...
					$sql = "UPDATE product SET stock = '$rem_qty' WHERE product_name = '".$ar_pro_name[$i]."'";
					$this->con->query($sql);
				}


				$insert_product = $this->con->prepare("INSERT INTO `sales_details`(`invoice_no`, `product_name`, `price`, `qty`)
				 VALUES (?,?,?,?)");
